...
export async function printActivitiesPDF({
  ...
  Object.values(enhancedOccurrences)
    .sort((a, b) => {
    ...
    .forEach((activity) => {
      ...
      drawCenteredText(
        `Activity ${activity.period[0]}${activity.period.length > 1 ? ` - ${activity.period[activity.period.length - 1]}` : ""}`,
        ypos,
        sansSerifFont,
        fontSize,
      );
      ...
      if (activity.headcount > 0) {
        ...
        const highlightText =
          activity.highlightedText.length > 0
            ? activity.highlightedText
            : "First & Last Name";
        const highlightWidth =
          sansSerifFont.widthOfTextAtSize(highlightText, fontSize) + 8;
